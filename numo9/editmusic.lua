...
function EditMusic:refreshSelectedMusic()
	local app = self.app
	local selMusic = app.audioBankPtr.musicAddrs + self.selMusicIndex
	...
	local ptr = ffi.cast('uint16_t*', app.audioBankPtr.audioData + selMusic.addr)
	local pend = ffi.cast('uint16_t*', app.audioBankPtr.audioData + selMusic.addr + selMusic.len)
	local nextTrack
	if ptr < pend then
		track.bps = ptr[0]
		ptr = ptr + 1
